...
class DummyDatabase {
    ...
    initializeData() {
        ...
        const teams = ["Ενωμένοι", "Σποριάδες", "Καρποφόροι", "Ολόφωτοι", "Νικητές", "Νικηφόροι", "Φλόγα", "Σύμψυχοι"];
        this.users = [
            // Admin user
            {
                uid: "admin-1",
            ...
            for (const userId of userIds) {
                ...
                for (let i = 0; i < jobsCount; i++) {
                    ...
                    for (let j = 0; j < numJobs; j++) {
                        ...
                        const prices = ((_a = this.priceTables.find((p) => p.id === "printing")) === null || _a === void 0 ? void 0 : _a.prices) || {};
                        let pricePerUnit = 0;
                        switch (type) {
                            case "A4BW":
                                pricePerUnit = prices.a4BW || 0;
                                break;
                            case "A4Color":
                                pricePerUnit = prices.a4Color || 0;
                                break;
                            case "A3BW":
                                pricePerUnit = prices.a3BW || 0;
                                break;
                            case "A3Color":
                                pricePerUnit = prices.a3Color || 0;
                                break;
                            case "RizochartoA3":
                                pricePerUnit = prices.rizochartoA3 || 0;
                                break;
                            case "RizochartoA4":
                                pricePerUnit = prices.rizochartoA4 || 0;
                                break;
                            case "ChartoniA3":
                                pricePerUnit = prices.chartoniA3 || 0;
                                break;
                            case "ChartoniA4":
                                pricePerUnit = prices.chartoniA4 || 0;
                                break;
                            case "Autokollito":
                                pricePerUnit = prices.autokollito || 0;
                                break;
                        }
                        const printJob = {
                ...
                for (let i = 0; i < laminationJobsCount; i++) {
                    const jobDate = new Date(now.getFullYear(), now.getMonth() - monthOffset, Math.floor(Math.random() * 28) + 1);
                    const types = ["A3", "A4", "A5", "cards", "spiral", "colored_cardboard", "plastic_cover"];
                    const type = types[Math.floor(Math.random() * types.length)];
                    const quantity = Math.floor(Math.random() * 3) + 1; // 1-3
                    const prices = ((_b = this.priceTables.find((p) => p.id === "lamination")) === null || _b === void 0 ? void 0 : _b.prices) || {};
    ...
    generateIncomePattern(user, totalDebt, billingDate) {
        const income = [];
        ...
        if (Math.random() < incomeProbability) {
            ...
            if (Math.random() < 0.7) {
                ...
                for (let i = 0; i < numPayments && remainingDebt > 0; i++) {
                    const incomeAmount = i === numPayments - 1
                        ? remainingDebt
                        : (0, utils_1.roundMoney)(remainingDebt * (0.3 + Math.random() * 0.4)); // 30-70% of remaining
            ...
            if (Math.random() < 0.3) {
                ...
                const lateIncomeAmount = (0, utils_1.roundMoney)(totalDebt * (0.5 + Math.random() * 0.3)); // 50-80% of debt
                income.push({
    ...
    refreshAllUserDebtFields() {
        this.initializeUserDebtFields();
    }
    updateUserDebtFields(uid) {
        const user = this.users.find(u => u.uid === uid);
        if (!user)
            return;
        // Recalculate debts using sequential rule
        const netDebt = this.getNetDebtForUser(uid);
        user.printDebt = (0, utils_1.roundMoney)(netDebt.print);
        user.laminationDebt = (0, utils_1.roundMoney)(netDebt.lamination);
        user.totalDebt = (0, utils_1.roundMoney)(netDebt.total);
    }
    ...
    regenerateIncome() {
        ...
        const manuallyAddedIncome = this.income.filter(income => income.timestamp > oneDayAgo);
        this.income = [];
        this.generateIncomeHistory();
...
exports.dummyDB = new DummyDatabase();
